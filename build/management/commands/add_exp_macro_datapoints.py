...
class Command(BaseCommand):
    ...
    Example row:
    molid, rawdataID, dataset, assigned_charge_state_transition, Exp_Macro_pKa_processed, T_processed, I_processed, assignment_method, assignment_error, primary_source, comments, exclude_flag
    ...
    Optional columns: T_processed, I_processed, primary_source, comments, exclude_flag.
    ...
    By default, missing intermediate charge states are automatically added. For example, if a molecule has
    charge states +1 and -1 after adding datapoints, a 0 charge state will be automatically created.
    Use the --no_consecutiveplaceholders flag to disable this behavior.
    """
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--no_consecutiveplaceholders', action='store_true', help='Do not add missing intermediate charge states automatically')

    def handle(self, *args, **kwargs):
        tablefile=kwargs['tablefile']
        separator=kwargs['separator']
        overwrite=kwargs['overwrite']
        no_consecutiveplaceholders=kwargs['no_consecutiveplaceholders']
        ...
        for row in df.iter_rows(named=True):
            ...
            if exclude_flag is not None and exclude_flag !='':
                if exclude_flag.lower().strip()=='exclude':
                    continue
            
            if row['molid'] is not None:
                molid = row['molid'].strip()
                molids_needed.add(molid)
            else:
                print(f"Error: 'molid' column is required: {row['rawdataID']}")
                continue
        ...
        for row in df.iter_rows(named=True):
            ...
            #skip if molid is None
            if molid is None:
                self.stdout.write(self.style.ERROR(f"'molid' column is required: {rawdataid}, skipping..."))
                continue
        ...
        with transaction.atomic():
            ...
            # Add missing intermediate charge states if requested
            if not no_consecutiveplaceholders:
                # Get all molecules that had datapoints added or overwritten
                molecules_to_check = set()
                for dp in data_to_create + data_to_overwrite:
                    molecules_to_check.add(dp.pre_charge_macrostate.molecule)

                if molecules_to_check:
                    # Bulk fetch all existing charge states for these molecules in a single query
                    all_existing_cms = ChargeMacroState.objects.filter(
                        molecule__in=molecules_to_check
                    ).select_related('molecule')

                    # Build a mapping: {molecule: set(charges)}
                    molecule_charges_map = {}
                    for cms in all_existing_cms:
                        if cms.molecule not in molecule_charges_map:
                            molecule_charges_map[cms.molecule] = set()
                        molecule_charges_map[cms.molecule].add(cms.charge)

                    # For each molecule, check if charge states are consecutive
                    cms_to_create_final = []
                    for molecule in molecules_to_check:
                        existing_charges = molecule_charges_map.get(molecule, set())

                        if not existing_charges:
                            continue

                        min_charge = min(existing_charges)
                        max_charge = max(existing_charges)
                        full_charge_set = set(range(min_charge, max_charge + 1))
                        missing_charges = full_charge_set - existing_charges

                        if missing_charges:
                            self.stdout.write(self.style.WARNING(f'For molecule {molecule.molid}, adding missing intermediate charge states: {sorted(missing_charges)}'))
                            for charge in missing_charges:
                                ...
                                cms_to_create_final.append(cms_obj)

                    if cms_to_create_final:
                        created_final_cms = ChargeMacroState.objects.bulk_create(cms_to_create_final)
                        self.stdout.write(self.style.SUCCESS(f'Created {len(created_final_cms)} missing intermediate ChargeMacroState entries.'))
